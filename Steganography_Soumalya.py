...
# Function to hash the passcode
def hash_password(password):
    return hashlib.sha256(password.encode()).hexdigest()[:10]  # First 10 chars as salt
...
def encrypt():
    ...
    try:
        ...
        msg = hashed_pass + msg  # Add hashed password to message

        msg_len_str = str(len(msg)).zfill(3)  # Ensure 3-digit length encoding
        n, m, z = 0, 0, 0  # Pixel location tracker

        # Store message length in first 3 pixels
        for i in range(3):
            img[n, m, z] = ord(msg_len_str[i])
            n, m, z = (n + 1) % img_height, (m + 1) % img_width, (z + 1) % 3

        # Store message content in subsequent pixels
        for char in msg:
            img[n, m, z] = ord(char)
            n, m, z = (n + 1) % img_height, (m + 1) % img_width, (z + 1) % 3
...
def decrypt():
    ...
    try:
        ...
        # Extract message length (first 3 pixels)
        msg_len_str = ""
        for i in range(3):
        ...
        try:
            msg_len = int(msg_len_str)  # Convert length string to integer
        except ValueError:
            messagebox.showerror("Error", "Failed to read message length. Image might be corrupted.")
            return
        ...
        stored_hashed_pass = extracted_msg[:10]  # First 10 chars store hashed password
        secret_message = extracted_msg[10:]  # Remaining chars are the actual message
...
def browse_image_enc():
    global img_path_enc
    img_path_enc = filedialog.askopenfilename(title="Select Image",
                                              filetypes=[("Image files", "*.jpg;*.jpeg;*.png")])
    if img_path_enc:
        original_img_path_label.config(text=f"Selected: {img_path_enc}")

# Function to select an image for decryption
def browse_image_dec():
    global img_path_dec
    img_path_dec = filedialog.askopenfilename(title="Select Image",
                                              filetypes=[("Image files", "*.png")])  # Only PNG
    if img_path_dec:
